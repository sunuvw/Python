...
def f1(a, b, c = 0, *args, **kw): # 以tuple的args()和dict的kw{}接收传入的参数
	print("a=", a, "b=", b, "c =", c, "args=", args, "kw=", kw)
def f2(a, b, c = 0, *, d, **kw):
	print("a=", a, "b=", b, "c=", c, "d=", d, "kw=", kw)
args = [4, 5, 6] # 或(4, 5, 6)
kw = {"d": 999, "name": "scc", "age": 28}
f1(1, 2)
f1(1, 2, 4, 5, 6, name = "scc", age = 28)
f1(1, 2, args[0], args[1], args[2], name = kw["name"], age = kw["age"])
f1(1, 2, *args, **kw ) # *number 里面的第一个参数，传入给了默认参数c，并不是作为整个的tuple(4, 5, 6)传入给可变参数
f1(1, 2, name = "scc", age = 28)
f1(*args, **kw)

f2(1, 2, d = "999", name = "scc", age = 28)
f2(*args, **kw) # dic里面存在"d": 999,所以可以传入给命名关键字参数

# 总结
# 默认参数一定要用不可变对象，如果是可变对象，程序运行时会有逻辑错误
# *args是可变参数，args接收的是一个tuple
# **kw是关键字参数，kw接收的是一个dict
# 可变参数可以直接传入：func(1, 2, 3),又可以先组装list或tuple，再通过*args传入：func(*(1, 2, 3)) 或func(*[1, 2, 3])
# 关键字参数既可以直接传入：func(a = 1, b = 2),又可以先组装dict，再通过**kw传入：func(**('a': 1, 'b': 2))
# 使用 *args和**kw作为参数名是python的习惯写法，也可以用其它名，但最好用习惯用法
# 命名的关键字参数是为了限制调用者可以传入的参数名，同时也可以提供默认值
#　定义命名的关键字参数在没有可变参数*args的情况下不要忘了写分隔符＊，否则定义的将是位置参数